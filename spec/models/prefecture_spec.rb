  ...
  describe '商品出品機能' do
    ...
    context '商品出品が出来ないとき' do
      ...
      it '商品の状態に「---」が選択されている場合は出品できない' do
        @product.status_id = Condition.find_by(name: '---').id
        @product.valid?
        expect(@product.errors.full_messages).to include "Status can't be blank"
      end
      it '配送料の負担に「---」が選択されている場合は出品できない' do
        @product.shopping_cost_id = Charges.find_by(name: '---').id
        @product.valid?
        expect(@product.errors.full_messages).to include "Shopping cost can't be blank"
      end
      it '発送元の地域に「---」が選択されている場合は出品できない' do
        @product.prefecture_id = Area.find_by(name: '---').id
        @product.valid?
        expect(@product.errors.full_messages).to include "prefecture can't be blank"
      end
      it '発送までの日数に「---」が選択されている場合は出品できない' do
        @product.shopping_day_id = Day.find_by(name: '---').id
        @product.valid?
        expect(@product.errors.full_messages).to include "Shopping_day can't be blank"
      end
